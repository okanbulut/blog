...
```{r catplot, fig.cap="Interim Theta and SEM Estimates in CAT"}
...
Figure \@ref(fig:catplot) shows that the SEM band around the theta estimate is getting narrower after administering each item, indicating that the theta estimate is becoming more precise. The final ability estimate is shown with "X" on the right-hand side of the figure. After 10 items, the final theta estimate appears to be higher than the true theta value (i.e. horizontal line). So, we may need to consider increasing the maximum number of items from 10 items to a larger value.  

To have a more detailed investigation of our CAT, we will use the simulateRespondents function and create a hypothetical CAT based on real responses in the data.  To keep our example simple, we will choose a random sample of 1000 persons (out of 4474 persons) and test the CAT system with this sample. 
