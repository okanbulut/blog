...
Using the item bank (i.e., full test) generated above, we want to create a short form with only 20 items. In the short form, we want to maintain the same content distribution. Therefore, we will have 5 items from each content area. Furthermore, we want the short form to resemble the full test with similar levels of reliability and correlations between scores. With the traditional test assembly approach, we would go through all of the items one by one and pick the appropriate ones based on our judgment. However, this would be highly laborious and inefficient in practice. Therefore, we will use two different approaches to automatically select the items based on the psychometric and content features we are looking for. 
...
To utilize MIP, we need a solver that will look for the optimal solution based on an objective function (e.g., maximizing test information function) and a set of constraints. In this example, for the objective function, we aim to maximize the test information between $\theta=-0.5$ and $\theta=0.5$ to mimic the TIF distribution from the full test (i.e,, 80 items). In addition, we will use **ata_constraint** to require five items from each content domain (i.e., A, B, C, and D). For the MIP solver, we will select [lp_solve](http://lpsolve.sourceforge.net/5.5/), which is already included in the `xxIRT` package. 
