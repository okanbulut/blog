...
description: |
  ...
  measurement instrument automaticallyin R. The first part focuses on a psychometric method 
...
**Assumptions:** We will assume that: (1) all of the items on the test measure the same latent trait (i.e., the test is unidimensional); (2) the short form will be used for the same examinee population; and (3) the test does not exhibit any psychometric issues (e.g., mode effect, context effect, or DIF).

**Conditions:** In the short form, we want to maintain the same content distribution (i.e., equal number of items from each content domain). Therefore, we will have 5 items from each content domain (i.e., A, B, C, and D). Furthermore, we want the short form to resemble the full test in terms of reliability and (raw) test scores. 
...
1. Automated test assembly as a psychometric approach (IRT parameters are required)
...
```{r, echo=TRUE, eval=TRUE}
# Run random forest for each content domain with a loop
result <- list() 
...
Both methods (i.e., ATA and RFE) appear to yield very similar (and very good!) results in terms of reliability and correlations among raw scores, although they recommended different sets of items. In this example, this may not be a surprising finding because we generated a nice data set (e.g., no missing responses, no outliers, etc.) based on a psychometric model (i.e., Rasch). Both ATA and RFE might produce less desirable results if:

* the sample size is smaller
* non-random missingness was in the response data set
