...
description: |
  ...
  the items one by one and choosing the appropriate ones based on personal judgment could highly 
...
With the mandatory transition to remote learning due to COVID-19, many educators around the world use online assessments (or e-assessments) because students are not able to physically attend classes in order to avoid the spread of the virus. When assessing student learning with online assessments, educators need to avoid long, heavily-weighted exams and instead administer shorter exams more frequently throughout the semester [@kuhfeld2020projecting]. Although this sounds like a good idea in theory, it is easier said than done in practice. 

To build shorter exams, educators first need to determine which items should be removed from the existing exams. In addition, they need to ensure that the reliability and other psychometric qualities (e.g., content distribution) of the shortened exam are acceptable. However, making such adjustments manually could be tedious and time-consuming. In this two-part series, I want to demonstrate how to shorten exams (or, any measurement instrument) by automatically selecting the most appropriate items. 
...
The reliability of the short form is quite high. Also, there is a very high correlation between the raw scores. Lastly, we will check the scatterplot of the raw scores from the full test (out of 80 points) and the short form (out of 20 points). 
...
Lastly, we can compute percent-correct scores for the full test and the short forms generated with ATA and RFE to make an overall score comparison. We will use the `ggplot2` package [@ggplot2] to draw a violin plot and check out the score distributions. 
