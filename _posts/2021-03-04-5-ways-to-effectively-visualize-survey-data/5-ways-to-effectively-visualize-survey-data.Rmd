...
```{r ch3, echo=FALSE, fig.width=5, fig.height=5, fig.cap="Correlation matrix plot of the PISA survey items"}
# Correlation matrix of items
cormat <- data %>%
  select(starts_with("ST0")) %>%
  cor(., use = "pairwise.complete.obs")
...
```{r ch7,echo=FALSE, fig.width=6, fig.height=5, layout="l-body-outset", fig.cap="An UpSet plot showing missing data patterns in the PISA dataset"}
gg_miss_upset(data[, 6:15], nsets = 10)
```
