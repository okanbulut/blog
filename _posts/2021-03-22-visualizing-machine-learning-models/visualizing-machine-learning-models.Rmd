...
description: |
 Data visualization plays an important role when evaluating the performance of machine learning models. In this post, we demonstrate how the **DALEX** package can be used for visualizing machine learning models. These visualizations can faciliate the comparison of machine learning models and help researchers understand which model works better and why.
  ```{r, include=FALSE}
...
First, let’s import the data into R and view its content
...
With the **DALEX** package, one of the things we can do is to analyze model performance as the distribution of residuals. Here, we consider the differences between observed and predicted probabilities as residuals. The `model_performance()` function calculates predictions and residuals for the testing data set.
...
According to boxplots of absolute residuals, RF seems to have the lowest median absolute residual value. Even though the GLM model has the highest AUC score, the RF model performs best when considering the median absolute residuals. We can also plot the distribution of residuals with histograms by setting the *geom="histogram"* and the precision recall curve by setting the *geom="prc"*.
...
Another function that calculates the importance of variables using the permutations is the `DALEX::model_parts()`. Let's use the default *loss_fuction* - One minus AUC - and set the *show_boxplots = FALSE* this time. Also, we limit the number of variables on the plot with the *max_vars* to show how to make readable results if there is a large number of variables in the dataset.
...
ALE plots are the extension of PDP, which is more suited for correlated variables. The `model_profile()` function with the parameter *type = "accumulated"* calculate the ALE curve. We recommend using ALE plots because features are usually correlated to some extent, and ALE plots are unbiased when features are correlated.
...
Using the **DALEX** we can see how the models behave for a single observation. We can select an observation from the data set or define a new observation. We investigate this using the *predict_parts()* function. This function is a special case of the `model_parts()`. `predict_parts()` calculates the importance of the variables for a single observation while `model_parts()` calculates it for all observations in the data set.

We show this single observation level explanation by using only the RF model. You can create the plots for each model and compare the importance of a selected variable across models. Let's first use one existing observation from the *test* set.

```{r}
student <- test[1, 1:11]
pp_rf <- predict_parts(explainer_rf, new_observation = student)
plot(pp_rf)
```

From the above plot, it seems that the prediction for the selected observation is 0.314. Also, the variable *grade* seems the most important one and decreases the prediction by 0.348. We can also define a new observation and investigate how the RF model behaves for this new observation.
...
For the new student we have defined, the prediction is 0.268. Also, the most important variable that decreases the prediction is *computer*. Setting the *type="shap"* we can inspect the contribution of variables to a single observation.
...
In the previous section, we have introduced the PDP plots. Ceteris Paribus Profiles (CPP) is the single observation level version of the PDP plots. For this we use `DALEX::predict_profile()` function. In this example, we show this plot by selecting a couple of variables from the data set for the same observation we have used before. You can plot the CPP for all variables. In the plot, *blue dots* represent the original value for the selected observation.

```{r}
selected_variables <- c("grade", "gender")
cpp_rf <- predict_profile(explainer_rf, student, variables = selected_variables)
